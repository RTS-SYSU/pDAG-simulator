...
public class PathGenerator {
	...
	public PathGenerator(int total_partitions, int totalTasks, boolean isHarmonic, boolean takeAllUtil,
			List<Double> assignedUtils, Random rng, boolean randomC, boolean print, List<int[]> level2) {

		this.totalUtil = SystemParameters.utilforTask * (double) totalTasks;
	...
	public Pair<List<DirectedAcyclicGraph>, CacheHierarchy> generatedDAGInstancesInOneHP(int forceInstanceNum,
			int hyperPeriodNum, long workload, List<Long> periods, boolean hard) {
		...
		this.workload = workload;// -1 to random
	...
	private List<DirectedAcyclicGraph> generateSporadicDAGs(List<Long> periods, boolean hard) {
		...
		// gyy: artificial set the branch workload
		if(workload != -1) schedParam.get(0).setWCET(workload);
		 ...
		 */
		for (int i = 0; i < total_tasks; i++) {
			StructuralParameters dag_param = new StructuralParameters(SystemParameters.CmaxLayer,
					SystemParameters.CminLayer, SystemParameters.CmaxParal, SystemParameters.CminParal,
					SystemParameters.CconnectProb, ran);
